...
#if defined(NDEBUG)
# error "Invicoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xff4d9a4e67add8b74f609e82e6f22dce0c3dca17742aa3dd365994e53132b745");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Invicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Invicoin Signed Message:\n";
...
{
    // Invicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 1 * 60; // Invicoin: 1 minute
static const int64 nTargetSpacing = 1 * 60; // Invicoin: 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Invicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CCheckProofOfWork(uint256 hash, unsigned int nBits)
{
    CBigNum bnTarget;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Invicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xff4d9a4e67add8b74f609e82e6f22dce0c3dca17742aa3dd365994e53132b745");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 22/Jun/2016 The New Panama Canal: A Risky Bet";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042118510fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1466589600;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1152765;
        ...
        assert(block.hashMerkleRoot == uint256("0x28af0dd58617506800fc09288fba33525ee2fab0606e335db507fd722d34d723"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xd2, 0xb6, 0xd2 }; // Invicoin: increase each by adding 2 to bitcoin's value.
...
CBlocksTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    // Create new block
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("InvicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("InvicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static InvicoinMiner(CWallet *pwallet)
{
    printf("InvicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("invicoin-miner");
    ...
    try { loop {
        ...
        printf("Running InvicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("InvicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&InvicoinMiner, pwallet));
}
